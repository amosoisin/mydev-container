...
### 2025-12-28 - 優先度:低タスク完了
- [x] compose.ymlにログ設定追加
  - json-fileドライバー + ローテーション（max-size: 10m, max-file: 3）
- [x] リソース制限の設定
  - 結論: 開発環境で常時ログイン使用のため不要
- [x] ヘルスチェック追加
  - 結論: 開発環境で常時ログイン使用のため不要
- [x] ログ集約の検討
  - 結論: 単一コンテナの個人開発環境のため現状維持（json-file + ローテーションで十分）
- [x] 各ツールの使い方ガイド作成
  - Neovim, Tmux, Lazygit, fzf, Rustツール群（ripgrep, fd, eza, bat, zoxide）、Claude Code
- [x] トラブルシューティングの充実
  - 9カテゴリ、15+の具体的な症状と解決方法を追加
...
**最終更新:** 2025-12-28（優先度:低タスク完了 - ログ設定追加、ドキュメント強化）
